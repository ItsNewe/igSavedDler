...
export class igInteractionModule {
	constructor() {
		dotenv.config();

		this.client.state.generateDevice("saturn");
		...
		this.client.request.end$.subscribe(async () => {
			...
			this.seshToFile(serialized);
		});
	}
	client: IgApiClient=new IgApiClient();
	user;
	SESSIONPATH: string="src/session.json";

	seshToFile(data) {
		fs.writeFile(this.SESSIONPATH, JSON.stringify(data), (e) => {
			if(e) {
				console.error(e);
				return false;
			}
	...
	loadSeshFromFile() {
		...
		try {
			let data: string=fs.readFileSync(this.SESSIONPATH, 'utf-8');
			if(data.length==0) {
				return null;
			} else {
	...
	async sessionLogin() {
		let sessionData=this.loadSeshFromFile();
	...
	async credentialsLogin() {
		try {
			...
			this.user=await this.client.account.login(process.env.USERNAME, process.env.PW);
			...
			if(e instanceof IgLoginTwoFactorRequiredError) {
				...
				console.log(this.client.state.checkpoint); // Checkpoint info here
				await this.client.challenge.auto(true); // Requesting sms-code or click "It was me" button
				console.log(this.client.state.checkpoint); // Challenge info here
				rl.question("Input verification code", async (answer) => {
					console.log(await this.client.challenge.sendSecurityCode(answer));
				});
			} else {
				console.error(e);
				return -1;
		...
		const savedFeed=this.client.feed.saved();
		let media=[];
